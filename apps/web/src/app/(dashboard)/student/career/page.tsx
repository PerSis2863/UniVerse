                ...
                {opportunities.map((job) => (
                  <div key={job.id} className="card p-0 overflow-hidden transition-all hover:border-indigo-500/50 group">
                    <div className="p-6">
                      <div className="flex flex-col sm:flex-row gap-4 justify-between items-start">
                        ...
                        <div className="flex flex-col sm:items-end gap-2 mt-4 sm:mt-0">
                          <span className="inline-block px-2.5 py-1 rounded-md text-xs font-medium bg-white/10 text-white">
                            {job.type}
                          </span>
                    ...
                    <div className="px-6 py-3 bg-white/[0.02] border-t border-white/[0.05] flex justify-end gap-3">
                      <button className="text-sm font-medium text-zinc-300 hover:text-white transition-colors">Save</button>
                      <button className="px-4 py-1.5 bg-indigo-500 hover:bg-indigo-600 text-white rounded-lg text-sm font-medium transition-colors">Apply Now</button>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="space-y-6">
              <div className="card">
                <div className="mb-4">
                  ...
                  <p className="text-sm text-zinc-400 mt-1">Don't miss these career events.</p>
                </div>
                <div className="space-y-4">
                  {events.map((event) => (
                    <div key={event.id} className="flex gap-4 border-b border-white/[0.05] last:border-0 pb-4 last:pb-0">
                      ...
                      <div>
                        <h4 className="font-semibold text-sm text-white line-clamp-2">{event.title}</h4>
                ...
                <button className="w-full mt-4 py-2 border border-white/10 rounded-lg text-sm font-medium text-white hover:bg-white/5 transition-colors flex items-center justify-center group">
