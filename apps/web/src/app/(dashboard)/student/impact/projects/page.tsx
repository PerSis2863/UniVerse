    ...
    <>
      ...
      <div className="flex-1 p-8 overflow-y-auto bg-zinc-50 dark:bg-zinc-950">
        <div className="max-w-7xl mx-auto space-y-8">
          ...
          {/* Banner */}
          <div className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-indigo-900/40 via-purple-950/30 to-amber-950/20 border border-zinc-200 dark:border-white/10 p-8 shadow-2xl">
            ...
            <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6 relative z-10">
              ...
              <div className="grid grid-cols-2 gap-3 w-full md:w-auto flex-shrink-0">
                <div className="bg-white dark:bg-zinc-900/80 border border-zinc-200 dark:border-zinc-800 rounded-xl p-4 text-center">
                  <div className="text-2xl font-black text-zinc-900 dark:text-white">48+</div>
                  <div className="text-[11px] text-zinc-600 dark:text-zinc-400 font-medium">Global Projects</div>
                </div>
                <div className="bg-white dark:bg-zinc-900/80 border border-zinc-200 dark:border-zinc-800 rounded-xl p-4 text-center">
                  <div className="text-2xl font-black text-emerald-400">$340K</div>
                  <div className="text-[11px] text-zinc-600 dark:text-zinc-400 font-medium">Micro-Grants Pool</div>
                </div>
              </div>
            </div>
          </div>
          ...
          {showSuccessToast && (
            <div className="p-4 rounded-xl bg-emerald-500/15 border border-emerald-500/30 text-emerald-300 flex items-center justify-between animate-in fade-in slide-in-from-top-4 duration-300">
              ...
              <button 
                ...
                className="text-emerald-400 hover:text-zinc-900 dark:text-white p-1"
              >
                <X className="w-4 h-4" />
              </button>
            </div>
                ...
                <button
                  ...
                  className={`px-3.5 py-1.5 rounded-lg text-xs font-medium whitespace-nowrap transition-all ${
                    selectedCategory === cat
                      ...
                      : 'bg-white dark:bg-zinc-900/60 border border-zinc-200 dark:border-zinc-800 text-zinc-600 dark:text-zinc-400 hover:text-zinc-900 dark:text-white hover:border-zinc-700'
                  }`}
                >
                  {cat}
                </button>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {filteredProjects.map(project => {
              ...
              return (
                <div
                  key={project.id}
                  className="bg-white dark:bg-zinc-900/40 border border-zinc-200 dark:border-zinc-800/80 rounded-2xl p-6 hover:border-indigo-500/40 transition-all flex flex-col justify-between group relative overflow-hidden"
                >
                  <div className="space-y-4">
                    {/* Header with NGO & SDG */}
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex items-center gap-3">
                        <div className={`w-10 h-10 rounded-xl bg-gradient-to-br ${project.ngo.logoBg} flex items-center justify-center text-zinc-900 dark:text-white font-black text-sm shadow-md flex-shrink-0`}>
                          {project.ngo.name.slice(0, 2).toUpperCase()}
                        </div>
                        <div>
                          <div className="flex items-center gap-1.5">
                            <span className="font-semibold text-zinc-900 dark:text-white text-sm">{project.ngo.name}</span>
                            {project.ngo.verified && (
                              <CheckCircle2 className="w-3.5 h-3.5 text-blue-400" />
                            )}
                          </div>
                          <span className="text-[11px] text-zinc-600 dark:text-zinc-400">{project.ngo.type}</span>
                        </div>
                      </div>
                    ...
                    {/* Title & Tagline */}
                    <div>
                      <h3 className="text-lg font-bold text-zinc-900 dark:text-white group-hover:text-indigo-300 transition-colors leading-snug mb-1">
                        {project.title}
                      </h3>
                    ...
                    <div className="space-y-1.5">
                      <div className="text-[11px] font-medium text-zinc-500 dark:text-zinc-500 uppercase tracking-wider">
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {project.partnerUniversities.map((uni, idx) => (
                          <span
                            key={idx}
                            className="text-xs px-2.5 py-0.5 rounded-md bg-zinc-100 dark:bg-zinc-800/60 border border-zinc-700/50 text-zinc-300 font-medium flex items-center gap-1"
                          >
                    ...
                    <div className="p-3 rounded-xl bg-zinc-50 dark:bg-zinc-950/60 border border-zinc-200 dark:border-zinc-800/60 flex items-center justify-between text-xs">
                      <span className="text-zinc-600 dark:text-zinc-400">Target Outcome:</span>
                      <span className="font-semibold text-emerald-400">{project.impactMetric}</span>
                    </div>
                  </div>

                  {/* Footer & Actions */}
                  <div className="pt-6 mt-6 border-t border-zinc-200 dark:border-zinc-800/60 flex items-center justify-between gap-4">
                    <div className="flex items-center gap-3 text-xs text-zinc-600 dark:text-zinc-400">
                      <span className="flex items-center gap-1">
                        <Users className="w-3.5 h-3.5 text-zinc-500 dark:text-zinc-500" />
                        <strong className="text-zinc-900 dark:text-white">{project.spotsLeft}</strong>/{project.totalSpots} spots
                      </span>
                      <span>•</span>
                      <span className="flex items-center gap-1">
                        <Clock className="w-3.5 h-3.5 text-zinc-500 dark:text-zinc-500" />
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        className="px-3.5 py-1.5 rounded-lg text-xs font-semibold bg-zinc-100 dark:bg-zinc-800 hover:bg-zinc-700 text-zinc-200 transition-colors"
                      >
                        Details
                      </button>
                      ...
                      <button
                        ...
                        className={`px-4 py-1.5 rounded-lg text-xs font-semibold transition-all flex items-center gap-1.5 ${
                          isApplied
                            ...
                            : 'bg-indigo-600 hover:bg-indigo-500 text-zinc-900 dark:text-white shadow-md shadow-indigo-600/30'
                        }`}
                      >
                        {isApplied ? (
                          <>
          ...
          {selectedProject && (
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-200">
              <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 w-full max-w-2xl rounded-2xl max-h-[90vh] overflow-y-auto shadow-2xl p-6 sm:p-8 space-y-6">
                ...
                <div className="flex items-start justify-between gap-4">
                  <div>
                    ...
                    <h2 className="text-2xl font-black text-zinc-900 dark:text-white">{selectedProject.title}</h2>
                    <p className="text-zinc-600 dark:text-zinc-400 text-sm mt-1">{selectedProject.tagline}</p>
                  </div>
                  <button
                    onClick={() => setSelectedProject(null)}
                    className="p-1 rounded-lg text-zinc-600 dark:text-zinc-400 hover:text-zinc-900 dark:text-white hover:bg-zinc-100 dark:bg-zinc-800 transition-colors"
                  >
                    <X className="w-5 h-5" />
                  </button>
                </div>

                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 p-4 bg-zinc-50 dark:bg-zinc-950/70 border border-zinc-200 dark:border-zinc-800 rounded-xl text-xs">
                  <div>
                    <span className="text-zinc-500 dark:text-zinc-500 block mb-1">Lead NGO</span>
                    <span className="font-semibold text-zinc-900 dark:text-white">{selectedProject.ngo.name}</span>
                  </div>
                  <div>
                    <span className="text-zinc-500 dark:text-zinc-500 block mb-1">Duration</span>
                    <span className="font-semibold text-zinc-900 dark:text-white">{selectedProject.duration}</span>
                  </div>
                  <div>
                    <span className="text-zinc-500 dark:text-zinc-500 block mb-1">Stipend & Award</span>
                    <span className="font-semibold text-emerald-400">{selectedProject.stipend}</span>
                  </div>
                </div>

                <div className="space-y-2">
                  <h4 className="text-xs font-semibold text-zinc-600 dark:text-zinc-400 uppercase tracking-wider">Project Overview</h4>
                  <p className="text-zinc-300 text-sm leading-relaxed">{selectedProject.description}</p>
                </div>

                <div className="space-y-2">
                  <h4 className="text-xs font-semibold text-zinc-600 dark:text-zinc-400 uppercase tracking-wider">Key Milestones & Deliverables</h4>
                ...
                <div className="p-4 rounded-xl bg-indigo-500/10 border border-indigo-500/20 flex items-center justify-between">
                  <div>
                    ...
                    <div className="text-sm font-bold text-zinc-900 dark:text-white">{selectedProject.facultyLead.name}</div>
                    <div className="text-xs text-zinc-600 dark:text-zinc-400">{selectedProject.facultyLead.title} ({selectedProject.facultyLead.uni})</div>
                  </div>
                  <UniverseLogo size="sm" animated={false} withGlow={false} />
                </div>
                ...
                <div className="pt-4 border-t border-zinc-200 dark:border-zinc-800 space-y-3">
                  ...
                  <textarea
                    ...
                    className="w-full p-3 bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-xl text-sm text-zinc-900 dark:text-white placeholder-zinc-600 focus:outline-none focus:border-indigo-500 transition-colors"
                  />
                  ...
                  <div className="flex items-center justify-end gap-3 pt-2">
                    ...
                    <button
                      ...
                      className="px-6 py-2.5 rounded-xl text-sm font-bold bg-indigo-600 hover:bg-indigo-500 text-zinc-900 dark:text-white shadow-lg shadow-indigo-600/30 flex items-center gap-2 transition-all"
                    >
