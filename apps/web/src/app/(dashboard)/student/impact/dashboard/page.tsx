    ...
    <>
      ...
      <div className="flex-1 p-8 overflow-y-auto bg-zinc-950">
        <div className="max-w-7xl mx-auto space-y-8">
          ...
          {/* Top Hero Card */}
          <div className="relative rounded-3xl bg-gradient-to-br from-[#0e1628] via-[#111827] to-[#1c1427] border border-white/10 p-8 overflow-hidden shadow-2xl">
            ...
            <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-8 relative z-10">
              <div className="space-y-4 max-w-xl">
                ...
                <div className="flex items-center gap-3 pt-2">
                  ...
                  <span className="text-xs text-zinc-400 font-medium">88.5% towards Master Fellow</span>
                </div>
              </div>

              {/* Logo Emblem Badge */}
              <div className="p-6 rounded-2xl bg-zinc-900/80 border border-white/10 flex flex-col items-center text-center space-y-3 shadow-xl">
                <UniverseLogo size="lg" animated={true} withGlow={true} />
                <div>
                  <div className="text-sm font-bold text-white">UniVerse Impact Fellow</div>
                  <div className="text-[11px] text-zinc-400">Institutional ID: #UV-2026-9042</div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {stats.map((s, idx) => (
              <div key={idx} className="bg-zinc-900/50 border border-zinc-800 p-5 rounded-2xl space-y-2">
          ...
          <div className="bg-zinc-900/40 border border-zinc-800 rounded-2xl p-6 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4">
              {sdgBadges.map((badge, idx) => (
                <div
                  key={idx}
                  className="bg-zinc-950/70 border border-zinc-800/80 rounded-xl p-4 text-center space-y-3 relative overflow-hidden group hover:border-indigo-500/40 transition-colors"
                >
                  <div className={`w-12 h-12 rounded-xl bg-gradient-to-br ${badge.color} mx-auto flex items-center justify-center text-white font-black text-lg shadow-md`}>
                    #{badge.num}
                  </div>
                  <div>
                    <h4 className="text-xs font-bold text-white line-clamp-1">{badge.name}</h4>
                    <span className="text-[10px] text-zinc-400">{badge.partner}</span>
                  </div>
                  <div className="pt-2 border-t border-zinc-800/60 flex items-center justify-between text-[11px]">
                    <span className="text-zinc-500">{badge.hours} hrs</span>
          ...
          <div className="bg-zinc-900/40 border border-zinc-800 rounded-2xl overflow-hidden">
            <div className="p-6 border-b border-zinc-800 flex items-center justify-between">
              <div>
                <h3 className="text-base font-bold text-white">Immutable Contribution Ledger</h3>
                <p className="text-xs text-zinc-400">All hours digitally signed by partner organizations.</p>
              </div>
              <span className="text-xs text-zinc-500 font-mono">Consortium Block #491,821</span>
            </div>

            <div className="divide-y divide-zinc-800/60">
              {impactActivities.map((act, i) => (
                <div key={i} className="p-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 hover:bg-zinc-800/20 transition-colors">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-zinc-400 mt-0.5">
                        {act.ngo} • <span className="text-zinc-500">{act.date}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-4 text-xs">
                    ...
                    <span className="font-mono text-zinc-500 hidden md:inline">{act.hash}</span>
          ...
          {showCertModal && (
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-200">
              <div className="bg-zinc-900 border border-zinc-800 w-full max-w-xl rounded-2xl p-8 space-y-6 shadow-2xl relative">
                ...
                <div className="border-2 border-indigo-500/30 rounded-2xl p-6 bg-gradient-to-br from-indigo-950/20 via-zinc-950 to-zinc-950 space-y-6 text-center relative">
                  ...
                  <div>
                    ...
                    <h2 className="text-2xl font-black text-white mt-1">Alex Rivera</h2>
                  ...
                  <div className="grid grid-cols-2 gap-4 py-4 border-y border-zinc-800 text-xs text-left">
                    <div>
                      <div className="text-zinc-500">Accreditation Body</div>
                      <div className="font-semibold text-white">UniVerse Academic Consortium</div>
                    </div>
                    <div>
                      <div className="text-zinc-500">UN SDG Alignments</div>
                      <div className="font-semibold text-white">SDG #3, #4, #6, #13, #17</div>
                    </div>
                  </div>
                ...
                <div className="flex items-center justify-end gap-3">
                  ...
                  <button
                    ...
                    className="px-5 py-2 rounded-xl text-sm font-bold bg-indigo-600 hover:bg-indigo-500 text-white flex items-center gap-2 shadow-lg shadow-indigo-600/30"
                  >
