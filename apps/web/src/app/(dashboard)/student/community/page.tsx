        ...
        <div className="max-w-5xl mx-auto flex flex-col lg:flex-row gap-8">
          ...
          <div className="flex-1 space-y-6">
            ...
            <div className="bg-zinc-900/50 border border-zinc-800 rounded-xl p-4 flex gap-4">
              ...
              <div className="flex-1">
                <input 
                  ...
                  className="w-full bg-zinc-800/50 border border-zinc-700 rounded-lg px-4 py-2.5 text-white placeholder:text-zinc-500 focus:outline-none focus:border-indigo-500 transition-colors"
                />
                <div className="flex justify-between items-center mt-3">
                  <div className="flex gap-2">
                    <button onClick={() => toast.success('Add Topic clicked')} className="text-zinc-400 hover:text-white text-sm font-medium px-2 py-1 rounded hover:bg-zinc-800 transition-colors">Add Topic</button>
                    <button onClick={() => toast.success('Attach File clicked')} className="text-zinc-400 hover:text-white text-sm font-medium px-2 py-1 rounded hover:bg-zinc-800 transition-colors">Attach File</button>
                  </div>
                  <button onClick={() => toast.success('Post submitted!')} className="bg-indigo-500 text-white px-4 py-1.5 rounded-lg text-sm font-medium hover:bg-indigo-600 transition-colors">Post</button>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-4">
              {MOCK_POSTS.map((post) => (
                <div key={post.id} className="bg-zinc-900/50 border border-zinc-800 rounded-xl p-5 hover:border-zinc-700 transition-colors">
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          ...
                          <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${
                            post.role === 'Teacher' ? 'bg-blue-500/10 text-blue-400' : 'bg-zinc-800 text-zinc-400'
                          }`}>{post.role}</span>
                        </div>
                  ...
                  <div className="flex gap-2 mb-4">
                    {post.tags.map((tag, i) => (
                      <span key={i} className="px-2.5 py-1 bg-zinc-800 text-zinc-300 text-xs rounded-md">
                        #{tag}
                      </span>
                    ))}
                  </div>
                  ...
                  <div className="flex items-center gap-6 pt-4 border-t border-zinc-800/50">
                    <button onClick={() => toast.success('Liked post!')} className="flex items-center gap-2 text-zinc-400 hover:text-red-400 transition-colors">
                      <Heart className="w-4 h-4" /> <span className="text-sm font-medium">{post.likes}</span>
                    </button>
                    <button onClick={() => toast.success('Viewing comments...')} className="flex items-center gap-2 text-zinc-400 hover:text-indigo-400 transition-colors">
                      <MessageSquare className="w-4 h-4" /> <span className="text-sm font-medium">{post.comments} Comments</span>
                    </button>
                    <button onClick={() => toast.success('Shared post!')} className="flex items-center gap-2 text-zinc-400 hover:text-white transition-colors ml-auto">
              ...
              <div className="space-y-3">
                {['#CS101', '#StudyGroup', '#CampusLife', '#Hackathon2026'].map((tag, i) => (
                  <div key={i} className="flex items-center justify-between group cursor-pointer">
                    ...
                    <span className="text-xs text-zinc-500">{120 - (i * 15)} posts</span>
                  </div>
                ))}
              </div>
            </div>
              ...
              <div className="space-y-4">
                {[
                  ...
                  <div key={i} className="flex items-center gap-3">
                    <div className={`w-8 h-8 rounded-full bg-gradient-to-br ${user.color} flex items-center justify-center text-white text-xs font-medium`}>
                      {user.name.charAt(0)}
                    </div>
                    <div>
                      <div className="text-sm font-medium text-zinc-200">{user.name}</div>
                      <div className="text-xs text-zinc-500">{user.posts} posts</div>
                    </div>
                  </div>
                ))}
              </div>
