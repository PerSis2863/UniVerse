      ...
      <div className="flex-1 p-8 overflow-y-auto">
        <div className="max-w-5xl mx-auto space-y-8">
          ...
          <div className="bg-gradient-to-r from-indigo-500/10 to-purple-500/10 border border-indigo-500/20 rounded-xl p-6 flex items-center gap-4">
            ...
            <div>
              <h3 className="font-semibold text-zinc-900 dark:text-white">University Directory of Services</h3>
              <p className="text-sm text-zinc-600 dark:text-zinc-400">These links are managed by the administration. For issues with these external portals, please contact the IT Helpdesk.</p>
            </div>
          </div>
              ...
              <div key={i} className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {section.items.map((item, j) => (
                    <a key={j} href={item.url} target="_blank" rel="noopener noreferrer" className="bg-white dark:bg-zinc-900/50 border border-zinc-200 dark:border-zinc-800 rounded-xl p-5 hover:border-zinc-600 hover:bg-zinc-100 dark:bg-zinc-800/50 transition-all group flex flex-col h-full">
                      <div className="flex justify-between items-start mb-2">
                        <h4 className="font-medium text-zinc-900 dark:text-white group-hover:text-indigo-400 transition-colors">{item.name}</h4>
                        <ExternalLink className="w-4 h-4 text-zinc-500 dark:text-zinc-500 group-hover:text-indigo-400 transition-colors flex-shrink-0" />
                      </div>
                      <p className="text-sm text-zinc-600 dark:text-zinc-400 mt-auto">{item.desc}</p>
                    </a>
                  ))}
                </div>
              </div>
