      ...
      <div className="flex-1 p-8 overflow-y-auto">
        <div className="max-w-6xl mx-auto space-y-6">
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            <button onClick={() => toast.success('My Applications clicked')} className="flex items-center gap-2 bg-indigo-500 hover:bg-indigo-600 text-white px-4 py-2 rounded-lg font-medium transition-colors whitespace-nowrap">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-6">
            {INTERNSHIPS.map((job) => (
              <div key={job.id} className="bg-zinc-900/50 border border-zinc-800 rounded-xl p-6 hover:border-zinc-700 transition-colors group">
                <div className="flex justify-between items-start mb-4">
                  <div className="flex gap-4">
                    ...
                    <div>
                      <h3 className="text-lg font-semibold text-white group-hover:text-indigo-400 transition-colors">{job.role}</h3>
                      <div className="flex items-center gap-2 text-sm text-zinc-400 mt-1">
                        <Building className="w-3.5 h-3.5" /> {job.company}
                      </div>
                    </div>
                  </div>
                  <button onClick={() => toast.success('Bookmark clicked')} className="text-zinc-500 hover:text-indigo-400 transition-colors">
                ...
                <div className="flex items-center justify-between pt-4 border-t border-zinc-800">
                  ...
                  <button onClick={() => toast.success('Apply Now clicked')} className="flex items-center gap-2 text-sm font-medium text-white hover:text-indigo-400 transition-colors">
