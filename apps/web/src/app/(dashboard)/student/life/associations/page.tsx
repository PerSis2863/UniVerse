    ...
    <>
      ...
      <div className="flex-1 p-8 overflow-y-auto">
        <div className="max-w-6xl mx-auto space-y-6">
          ...
          {loading ? (
            <div className="text-zinc-400">Loading associations...</div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {filteredAssociations.map(assoc => {
                const isMember = myMemberships.has(assoc.id);
                return (
                  <div key={assoc.id} className="bg-zinc-900/50 border border-zinc-800 p-6 rounded-xl hover:border-zinc-700 transition-colors flex flex-col">
                    ...
                    <div className="space-y-3 pt-4 border-t border-zinc-800/50 flex-1">
                      <p className="text-sm text-zinc-400">{assoc.description}</p>
                    </div>
