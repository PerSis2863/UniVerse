          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            ...
            <div className="flex gap-2 w-full sm:w-auto">
              ...
              <div className="relative">
                ...
                {showFilterDropdown && (
                  <div className="absolute right-0 top-11 z-30 bg-zinc-900 border border-zinc-700 rounded-xl shadow-2xl w-40 py-1 animate-in fade-in slide-in-from-top-2 duration-150">
                    {STATUSES.map(s => (
                      <button
                        key={s}
                        onClick={() => { setStatusFilter(s); setShowFilterDropdown(false); }}
                        className={`w-full text-left px-4 py-2 text-sm transition-colors ${
                          statusFilter === s ? 'text-indigo-400 bg-indigo-500/10' : 'text-zinc-300 hover:bg-zinc-800'
                        }`}
                      >
                        {s}
                      </button>
          ...
          {filtered.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filtered.map((project) => (
                <div key={project.id} className="bg-zinc-900/50 border border-zinc-800 rounded-xl p-6 hover:border-zinc-700 transition-colors flex flex-col group">
          ...
          <div className="bg-zinc-900 border border-zinc-800 w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden">
            <div className="p-6 border-b border-zinc-800 flex items-start justify-between">
              <div>
                ...
                <p className="text-sm text-zinc-400 mt-1">{selectedProject.partner}</p>
              </div>
              <button onClick={() => setSelectedProject(null)} className="p-1.5 rounded-lg text-zinc-400 hover:text-white hover:bg-zinc-800 transition-colors">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="p-6 space-y-5">
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div className="p-3 bg-zinc-800/50 rounded-xl">
                  <div className="text-zinc-500 text-xs mb-1">Students Assigned</div>
                  <div className="font-bold text-white text-lg">{selectedProject.students}</div>
                </div>
                <div className="p-3 bg-zinc-800/50 rounded-xl">
                  <div className="text-zinc-500 text-xs mb-1">Deadline</div>
                  <div className="font-bold text-white">{selectedProject.deadline}</div>
                </div>
              </div>

              <div>
                <div className="text-xs font-semibold text-zinc-400 uppercase tracking-wider mb-2">Description</div>
                <p className="text-sm text-zinc-300 leading-relaxed">{selectedProject.description}</p>
              </div>

              <div>
                <div className="text-xs font-semibold text-zinc-400 uppercase tracking-wider mb-2">Milestones</div>
              ...
              <div className="p-3 bg-zinc-800/40 rounded-xl text-sm flex items-center justify-between">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
          <div className="bg-zinc-900 border border-zinc-800 w-full max-w-lg rounded-2xl shadow-2xl p-6 space-y-5">
            <div className="flex items-start justify-between">
              <div>
                <h2 className="text-xl font-bold text-white">Propose New NGO Project</h2>
                <p className="text-sm text-zinc-400 mt-1">Submit a new mentorship initiative for consortium review.</p>
              </div>
              <button onClick={() => setShowProposeModal(false)} className="p-1.5 rounded-lg text-zinc-400 hover:text-white hover:bg-zinc-800 transition-colors">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="text-xs font-medium text-zinc-300 block mb-1">Required Skills <span className="text-zinc-500">(comma-separated)</span></label>
                <input
                  type="text"
                  value={proposeTags}
            ...
            <div className="flex items-center justify-end gap-3 pt-2 border-t border-zinc-800">
              <button onClick={() => setShowProposeModal(false)} className="px-4 py-2 text-sm text-zinc-400 hover:text-white transition-colors">
                Cancel
              </button>
              <button
