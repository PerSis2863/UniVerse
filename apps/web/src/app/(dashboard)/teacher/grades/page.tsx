      ...
      <div className="flex-1 p-8 overflow-y-auto space-y-6">
        ...
        <div className="flex flex-col md:flex-row gap-4 justify-between items-start md:items-center">
          <div className="flex gap-4 items-center">
            <select 
              className="bg-zinc-900 border border-zinc-800 rounded-xl px-4 py-2.5 text-white focus:outline-none focus:border-indigo-500 font-medium"
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-zinc-800 bg-zinc-900/80">
                  ...
                  <th className="p-4 text-xs font-bold text-zinc-500 uppercase tracking-wider text-center">Assignments</th>
                  <th className="p-4 text-xs font-bold text-zinc-500 uppercase tracking-wider text-center">Midterm</th>
                  <th className="p-4 text-xs font-bold text-zinc-500 uppercase tracking-wider text-center">Final</th>
                  <th className="p-4 text-xs font-bold text-zinc-500 uppercase tracking-wider text-center">Total Grade</th>
              ...
              <tbody className="divide-y divide-zinc-800/50">
                {filteredGrades.map((grade) => (
                  <tr key={grade.id} className="hover:bg-zinc-800/30 transition-colors group">
                    <td className="p-4">
                      <div className="flex items-center gap-3">
                        <div className="w-9 h-9 rounded-full bg-gradient-to-br from-indigo-500 to-purple-500 flex items-center justify-center text-white text-sm font-medium shadow-sm">
                          {grade.studentName.charAt(0)}
                        </div>
                        <div>
                          <div className="text-sm font-medium text-white">{grade.studentName}</div>
                          <div className="text-xs text-zinc-500">{grade.email}</div>
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="p-4 text-center">
                      <div className="flex flex-col items-center justify-center gap-1">
                        <span className="font-bold text-white text-lg">{grade.total}%</span>
                        {grade.trend === 'up' ? (
                    ...
                    <td className="p-4 text-right">
                      <button onClick={() => toast.success(`Viewing full details for ${grade.studentName}`)} className="p-2 text-zinc-400 hover:text-white rounded-lg hover:bg-zinc-800 transition-colors opacity-0 group-hover:opacity-100 focus:opacity-100">
