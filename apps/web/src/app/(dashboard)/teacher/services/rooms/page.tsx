      ...
      <div className="flex-1 p-8 overflow-y-auto">
        <div className="max-w-4xl mx-auto space-y-8">
          ...
          <div className="bg-gradient-to-r from-blue-500/10 to-indigo-500/10 border border-blue-500/20 rounded-xl p-6 flex gap-6 items-center">
            ...
            <div>
              <h2 className="text-xl font-bold text-white mb-1">Need a space for a special session?</h2>
              <p className="text-zinc-400 text-sm max-w-2xl">Reserve large lecture halls, seminar rooms, and labs for extra classes or exams.</p>
            </div>
          </div>
          ...
          <div className="bg-zinc-900/50 border border-zinc-800 rounded-xl p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
              ...
              <div className="space-y-2">
                ...
                <select value={roomType} onChange={e => setRoomType(e.target.value)} className="w-full bg-zinc-800 border border-zinc-700 rounded-lg px-3 py-2 text-white appearance-none focus:outline-none focus:border-indigo-500 transition-colors">
              ...
              {filteredRooms.map((room) => (
                <div key={room.id} className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-4 bg-zinc-900/50 border border-zinc-800 rounded-xl hover:border-zinc-700 transition-colors gap-4">
                  <div className="flex-1">
                    ...
                    <div className="text-sm text-zinc-400">Capacity: {room.capacity} people • {room.amenities}</div>
                  </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
          <div className="bg-zinc-900 border border-zinc-800 w-full max-w-sm rounded-2xl shadow-2xl p-6 space-y-4">
            <div className="flex items-start justify-between">
              ...
              <button onClick={() => setConfirmBooking(null)} className="p-1 text-zinc-400 hover:text-white"><X className="w-5 h-5" /></button>
            </div>
            ...
            <div>
              ...
              <input type="text" value={purpose} onChange={e => setPurpose(e.target.value)} placeholder="e.g., Midterm Exam CS401" className="w-full px-3 py-2 bg-zinc-950 border border-zinc-800 rounded-xl text-sm text-white focus:outline-none focus:border-indigo-500" />
            </div>
