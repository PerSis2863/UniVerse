...
const typeColors: Record<string, string> = {
  ...
  'Upcoming': 'bg-zinc-500/10 text-zinc-400',
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            {stats.map(stat => (
              <div key={stat.label} className="bg-zinc-900/50 border border-zinc-800 p-6 rounded-xl">
                <div className="text-sm font-medium text-zinc-400 mb-2">{stat.label}</div>
          ...
          <div className="bg-zinc-900/50 border border-zinc-800 rounded-xl overflow-hidden">
            <div className="p-4 border-b border-zinc-800 flex justify-between items-center bg-zinc-900/80">
              <h3 className="font-semibold text-white">
                ...
                <span className="ml-2 text-sm font-normal text-zinc-500">({filteredBookings.length})</span>
              </h3>
              <div className="relative">
                ...
                {showFilterDropdown && (
                  <div className="absolute right-0 top-10 z-30 bg-zinc-900 border border-zinc-700 rounded-xl shadow-2xl w-48 py-1 animate-in fade-in slide-in-from-top-2 duration-150">
                    {BOOKING_TYPES.map(t => (
                      <button
                        key={t}
                        onClick={() => { setTypeFilter(t); setShowFilterDropdown(false); }}
                        className={`w-full text-left px-4 py-2 text-sm transition-colors ${
                          typeFilter === t ? 'text-indigo-400 bg-indigo-500/10' : 'text-zinc-300 hover:bg-zinc-800'
                        }`}
                      >
                        {t}
                      </button>
            ...
            <div className="divide-y divide-zinc-800/50">
              {filteredBookings.length === 0 ? (
                ...
                filteredBookings.map((booking) => (
                  <div key={booking.id} className="flex flex-col sm:flex-row sm:items-center justify-between p-4 hover:bg-zinc-800/20 transition-colors gap-4">
                    <div className="flex items-start gap-4">
                      ...
                      <div>
                        <div className="flex items-center gap-2 mb-1">
                          <h4 className="font-medium text-white">{booking.room}</h4>
                          <span className={`text-xs px-2 py-0.5 rounded-full ${typeColors[booking.type]}`}>
                            {booking.type}
                          </span>
                        </div>
                        <div className="text-sm text-zinc-400">
                          {booking.user} • <span className="text-zinc-500">{booking.purpose}</span>
                        </div>
      ...
      {confirmCancel && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
          <div className="bg-zinc-900 border border-zinc-800 w-full max-w-sm rounded-2xl shadow-2xl p-6 space-y-4">
            ...
            <h2 className="text-lg font-bold text-white text-center">Cancel Booking?</h2>
            <p className="text-sm text-zinc-400 text-center">The user will be notified that their room reservation has been cancelled.</p>
            <div className="flex gap-3">
              <button onClick={() => setConfirmCancel(null)} className="flex-1 px-4 py-2 text-sm text-zinc-400 hover:text-white bg-zinc-800 hover:bg-zinc-700 rounded-xl transition-colors">Keep</button>
              <button onClick={() => handleCancel(confirmCancel)} className="flex-1 px-4 py-2 text-sm font-bold text-white bg-rose-600 hover:bg-rose-500 rounded-xl transition-colors">Cancel Booking</button>
            </div>
          </div>
        </div>
      )}
