      ...
      {isStripeModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl w-full max-w-md shadow-2xl p-6 relative">
            <button onClick={() => setIsStripeModalOpen(false)} className="absolute top-4 right-4 text-zinc-400 hover:text-white transition-colors">
              <X className="w-5 h-5" />
            </button>
            <div className="flex flex-col items-center text-center space-y-4 pt-4">
              ...
              <h2 className="text-xl font-bold text-white">Connect Stripe</h2>
              ...
              <button 
                ...
                className="w-full py-3 bg-[#635BFF] hover:bg-[#635BFF]/90 text-white rounded-xl font-medium transition-colors shadow-lg shadow-[#635BFF]/20 mt-4"
              >
                Connect with Stripe
              </button>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl w-full max-w-md shadow-2xl overflow-hidden">
            <div className="flex justify-between items-center p-6 border-b border-zinc-800">
              <h2 className="text-lg font-semibold text-white">Manual Transaction</h2>
              <button onClick={() => setIsAddTrxModalOpen(false)} className="text-zinc-400 hover:text-white transition-colors">
                <X className="w-5 h-5" />
              </button>
            </div>
            <form onSubmit={handleAddTransaction} className="p-6 space-y-4">
              <div className="space-y-2">
                <label className="text-sm font-medium text-zinc-400">User / Client</label>
                <input required value={formData.user} onChange={e => setFormData({...formData, user: e.target.value})} type="text" className="w-full bg-zinc-950 border border-zinc-800 rounded-lg px-4 py-2 text-white outline-none focus:border-indigo-500" placeholder="e.g. John Doe" />
              </div>
              <div className="space-y-2">
                <label className="text-sm font-medium text-zinc-400">Transaction Type</label>
                <select value={formData.type} onChange={e => setFormData({...formData, type: e.target.value})} className="w-full bg-zinc-950 border border-zinc-800 rounded-lg px-4 py-2 text-white outline-none focus:border-indigo-500">
              ...
              <div className="space-y-2">
                ...
                <input required value={formData.amount} onChange={e => setFormData({...formData, amount: Number(e.target.value)})} type="number" step="0.01" className="w-full bg-zinc-950 border border-zinc-800 rounded-lg px-4 py-2 text-white outline-none focus:border-indigo-500" />
              </div>
              <div className="space-y-2">
                <label className="text-sm font-medium text-zinc-400">Status</label>
                <select value={formData.status} onChange={e => setFormData({...formData, status: e.target.value})} className="w-full bg-zinc-950 border border-zinc-800 rounded-lg px-4 py-2 text-white outline-none focus:border-indigo-500">
        ...
        <div className="max-w-7xl mx-auto space-y-8">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-gradient-to-r from-indigo-500/10 via-purple-500/5 to-transparent border border-indigo-500/20 p-5 rounded-2xl backdrop-blur-xl">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h2 className="text-white font-semibold">Financial Dashboard</h2>
                <span className="text-zinc-400 text-sm">Real-time revenue metrics and transactions</span>
              </div>
            </div>
            <div className="flex gap-3 w-full sm:w-auto">
              <button onClick={() => setIsStripeModalOpen(true)} className="flex-1 sm:flex-none flex justify-center items-center gap-2 px-5 py-2.5 bg-zinc-900 border border-zinc-700 text-zinc-300 rounded-xl hover:bg-zinc-800 hover:text-white transition-all shadow-sm">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {STATS.map((stat, i) => (
              <div key={i} className="bg-zinc-900/40 backdrop-blur-xl border border-zinc-800/50 rounded-2xl p-6 relative overflow-hidden group hover:bg-zinc-900/80 transition-all duration-300">
                ...
                <div className="text-3xl font-bold text-white mb-3 tracking-tight">{stat.value}</div>
                ...
                <div className={`flex items-center text-xs font-semibold px-2.5 py-1 rounded-full w-max ${
                  ...
                  {stat.trend} <span className="text-zinc-500 font-normal ml-1">vs last month</span>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="bg-zinc-900/40 backdrop-blur-xl border border-zinc-800/50 rounded-2xl p-8 relative overflow-hidden">
            ...
            <div className="flex justify-between items-center mb-8">
              <div>
                ...
                <p className="text-sm text-zinc-400">Monthly breakdown of gross volume</p>
              </div>
              <select className="bg-zinc-950 border border-zinc-800 text-sm text-zinc-300 rounded-lg px-3 py-1.5 outline-none focus:border-indigo-500">
                <option>2026</option>
                <option>2025</option>
              </select>
            </div>
            
            <div className="h-64 flex items-end justify-between gap-3 border-b border-zinc-800/50 pb-2 relative mt-4">
              {/* Y-axis lines */}
              <div className="absolute inset-0 flex flex-col justify-between pointer-events-none">
                {[4,3,2,1,0].map(i => (
                  <div key={i} className="w-full border-t border-zinc-800/30 flex items-start">
              ...
              {[40, 60, 45, 80, 55, 90, 75, 100, 85, 110, 95, 120].map((h, i) => (
                <div key={i} className="w-full bg-gradient-to-t from-indigo-600/20 to-indigo-400/40 hover:from-indigo-500/40 hover:to-indigo-300/60 border-t border-x border-indigo-400/30 rounded-t-md transition-all duration-300 relative group cursor-pointer" style={{ height: `${h}%` }}>
                  <div className="absolute -top-10 left-1/2 -translate-x-1/2 bg-zinc-800 text-white text-xs font-medium py-1 px-2.5 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none shadow-xl border border-zinc-700 whitespace-nowrap z-10">
                    ${(h * 300).toFixed(0)}
                  </div>
                </div>
              ))}
            </div>
            <div className="flex justify-between mt-4 text-xs font-medium text-zinc-500 pl-11">
          ...
          <div className="bg-zinc-900/40 backdrop-blur-xl border border-zinc-800/50 rounded-2xl overflow-hidden">
            <div className="p-6 border-b border-zinc-800/50 flex justify-between items-center">
              <div>
                ...
                <p className="text-sm text-zinc-400">Latest activity across your platform</p>
              </div>
              <button 
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-zinc-950/50">
                    <th className="p-4 text-xs font-semibold text-zinc-500 uppercase tracking-wider border-b border-zinc-800/50">Transaction ID</th>
                    <th className="p-4 text-xs font-semibold text-zinc-500 uppercase tracking-wider border-b border-zinc-800/50">Date</th>
                    <th className="p-4 text-xs font-semibold text-zinc-500 uppercase tracking-wider border-b border-zinc-800/50">Type</th>
                    <th className="p-4 text-xs font-semibold text-zinc-500 uppercase tracking-wider border-b border-zinc-800/50">User</th>
                    <th className="p-4 text-xs font-semibold text-zinc-500 uppercase tracking-wider border-b border-zinc-800/50">Amount</th>
                    <th className="p-4 text-xs font-semibold text-zinc-500 uppercase tracking-wider border-b border-zinc-800/50">Status</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-zinc-800/30">
                  {transactions.map((trx, i) => (
                    <tr key={i} className="hover:bg-zinc-800/20 transition-colors group">
                      <td className="p-4 font-mono text-xs font-medium text-zinc-400 group-hover:text-zinc-300">{trx.id}</td>
                      <td className="p-4 text-sm text-zinc-400">{trx.date}</td>
                      <td className="p-4 text-sm font-medium text-white">{trx.type}</td>
                      <td className="p-4 text-sm text-zinc-300">{trx.user}</td>
                      <td className={`p-4 text-sm font-semibold ${trx.amount > 0 ? 'text-emerald-400' : 'text-white'}`}>
